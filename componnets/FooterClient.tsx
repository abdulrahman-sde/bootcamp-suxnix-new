...
export default function FooterClient({ data }: FooterClientProps) {
  ...
  console.log(path);
  return (
    <footer className={`text-white} overflow-hidden bg-[#0A0A0A]`}>
      {/* Decorative top brush image */}
      <div className="w-full">
        <PrismicNextImage
          field={data.top_reshaper}
          className={`w-full object-cover ${path === "/features" ? "" : ""}`}
        />
        <PrismicNextImage
          field={data.leaves_1}
      ...
      <Container
        className={`relative z-10 mx-auto grid grid-cols-1 gap-20 px-6 py-12 md:grid-cols-4`}
      >
